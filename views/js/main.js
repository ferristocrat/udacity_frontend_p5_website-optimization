...
var pizzaElementGenerator = function(i) {
  ...
  pizzaContainer.style.width = "390px";
...
var resizePizzas = function(size) {
  ...
  // Changes the value for the size of the pizza above the slider as well as the size of the pizzas
  var pizzaContainerList = document.getElementsByClassName("randomPizzaContainer");
  var lengthList = pizzaContainerList.length;

  var assignWidth = function(pizzaWidth) {
...
function updatePositions() {
  ...
  var phase = [];
  for (i=0; i < 5; i++) {
    phase[i] = Math.sin((position / 1250) + (i % 5));
  }
  for (var i = 0; i < pizzas.length; i++) {
    pizzas[i].style.left = ((i % 8) * 256) + 100 * phase[i % 5] + 'px';
  }
